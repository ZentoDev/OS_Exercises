...
//   cin >> portaServizio;
   ...
   if(clientSocket != -1)
   {
      ...
      do {
         char buf[DIM_BUFFER];
         cout << "In attesa dei dati\n";
         int mex = recv(clientSocket, buf, strlen(buf), 0);

         cout << "dati ricevuti, stampa del messaggio:\n";
         cout << buf << "\n";

         cout << "Comunicazione completata, chiudere la connessione? (1 per uscire) \n";
         cin >> stop;
